# Changelog

All notable changes are documented in this file.

## [Unreleased]

### Added

- Installable `src/` package and unified `borehole-fracture` CLI.
- Automated checks, tests, CI configuration, architecture and data documentation.
- Code-only MIT licensing boundary and local-data exclusion rules.

### Changed

- Renamed attachment folders and problem-number scripts to semantic English names.
- Separated raw data, training data, model checkpoints, and generated artifacts.

## [0.1.0] - 2026-08-16

- Initial open-source project structure.
